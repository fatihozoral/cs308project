...
async def get_approved_comments(user=Depends(get_current_user)):
    """Get all approved comments for homepage display"""
    res = supabase.table("comments").select("*").eq("status", "approved").order("created_at", desc=True).limit(12).execute()
    return res.data or []
